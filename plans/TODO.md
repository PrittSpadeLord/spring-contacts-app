1. Set up tests for the remaining services and components.
2. Set up Docker with a docker compose to run a `Dockerfile.test` before the real `Dockerfile` with necessary environmental variables.
3. Write up plan for "Logging in" functionality and implement it.
4. Research on how to test for controllers and other classes. Also research on how we can incorporate IoC in Tests instead of manually initializing them.
5. Bucket map need to be cleaned to remove inactive Buckets